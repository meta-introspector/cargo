...
pub fn generate_submodule_patches(
    ...
    for sm_info in GitRepositoryOperations::submodules(&repo).map_err(|e| e.to_string())? {
        submodule_name_to_path.insert(sm_info.name.clone(), current_dir.join(&sm_info.path));
    }

    for submodule_info in GitRepositoryOperations::submodules(&repo).map_err(|e| e.to_string())?
    {
